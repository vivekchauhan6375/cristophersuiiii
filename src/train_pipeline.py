...
# Initialize variables for layers and gradients
z = [None] * config.NUM_LAYERS
h = [None] * config.NUM_LAYERS
...
def del_layer_neurons_outputs_wrt_weighted_sums(current_layer_neurons_activation_function, current_layer_neurons_weighted_sums):
    if current_layer_neurons_activation_function == "linear":
        ...
        return 1 - (layer_neurons_output(current_layer_neurons_weighted_sums, current_layer_neurons_activation_function) ** 2)
    elif current_layer_neurons_activation_function == "relu":
        return 1. * (current_layer_neurons_weighted_sums > 0)
    ...
    while True:
        ...
        for batch in range(num_batches):
            ...
            for i in range(X_batch.shape[0]):
                ...
                for l in range(1, config.NUM_LAYERS):
                    z[l] = layer_neurons_weighted_sum(h[l-1], pl.theta0[l], pl.theta[l])
                    h[l] = layer_neurons_output(z[l], config.STRUCTURE[l]['activation'])
                    del_fl_by_del_z[l] = del_layer_neurons_outputs_wrt_weighted_sums(config.STRUCTURE[l]['activation'], z[l])
                    del_hl_by_del_theta0[l] = 1
                    del_hl_by_del_theta[l] = h[l-1]
